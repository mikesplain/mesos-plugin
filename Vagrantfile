...
$script = <<SCRIPT
 ...
 ln -s -f /usr/lib/jvm/java-6-openjdk-amd64/jre/lib/amd64/server/libjvm.so /usr/lib/libjvm.so
 ...
 # Acquire mesos-jenkins plugin code, either from
 #  - latest local source on host machine (MESOS_PLUGIN_VERSION="local") - helpful for testing local changes on VM
 #  - latest configured release download  (MESOS_PLUGIN_VERSION="x.x.x")

 # MESOS_PLUGIN_VERSION="local"
 MESOS_PLUGIN_VERSION="0.5.0"

 if [ $MESOS_PLUGIN_VERSION = "local" ]; then
    rm -rf mesos-plugin-mesos-${MESOS_PLUGIN_VERSION}
    mkdir mesos-plugin-mesos-${MESOS_PLUGIN_VERSION}
